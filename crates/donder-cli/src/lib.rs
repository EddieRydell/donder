...
use donder_package::{
    AssetDeclaration, AssetKind, CacheStore, CandidateResolution, Dependency, DonderDirectories,
...
#[derive(Debug, Parser)]
#[command(name = "donder", version, about = "Donder projects and packages")]
pub struct Cli {
...
pub enum CliError {
    ...
    Project(donder_project_io::PackageLoadError),
}
...
impl From<donder_project_io::PackageLoadError> for CliError {
    fn from(error: donder_project_io::PackageLoadError) -> Self {
        Self::Project(error)
    }
}
...
fn init_with_registry(root: &Utf8Path, registry: &str) -> Result<(), CliError> {
    ...
    if root.join(donder_package::MANIFEST_FILE).exists() {
        return Err(PackageError::Invalid(format!(
            "{} already exists",
            root.join(donder_package::MANIFEST_FILE)
        ))
        .into());
    }
    let documents = donder_documents(root)?;
    if documents.is_empty() {
        return Err(PackageError::Invalid(
            "donder init requires at least one existing .donder document".to_string(),
        )
        .into());
    }
    let project = documents
        .iter()
        .find(|path| path.as_str() == "project.donder")
    ...
    println!("initialized {}", root.join(donder_package::MANIFEST_FILE));
    Ok(())
}

fn check(root: &Utf8Path) -> Result<(), CliError> {
    let compiled = donder_project_io::compile_package(root)?;
...
fn accept_package_candidate(
    ...
    let candidate = PackageService::prepare(
        root,
        resolution,
        donder_project_io::validate_registry_package_artifact,
    )?;
    for package in &candidate.deprecated_packages {
        eprintln!("warning: package `{package}` is deprecated");
    }
    let _ = donder_project_io::compile_package_with_cache(
        root,
        candidate.manifest,
    ...
    println!("synchronized {}", root.join(donder_package::LOCK_FILE));
    Ok(())
}
    ...
    if let Err(operation_error) = sync(root, SyncMode::Resolve) {
        original.write(root).map_err(|rollback_error| {
            PackageError::Invalid(format!(
                "dependency change failed ({operation_error}) and restoring donder-package.json also failed: {rollback_error}"
            ))
        })?;
...
fn pack(root: &Utf8Path) -> Result<(), CliError> {
    let release = donder_project_io::pack_package(root)?;
...
fn write_release_artifact(
    ...
    let directory = root.join("target").join("donder");
    ...
    if path.is_file() {
        ...
        if donder_package::sha256_hex(&existing) != release.archive_sha256 {
            return Err(PackageError::Invalid(format!(
    ...
    donder_package::atomic_write(&path, &release.archive)?;
    Ok(path)
}
...
fn donder_documents(root: &Utf8Path) -> Result<Vec<Utf8PathBuf>, CliError> {
    let mut files = Vec::new();
    collect_matching_files(root, root, &mut files, |path| path.ends_with(".donder"))?;
    Ok(files)
}
...
pub(crate) fn cache() -> Result<CacheStore, CliError> {
    Ok(DonderDirectories::discover()?.package_cache())
}
...
mod tests {
    ...
    fn clap_exposes_the_complete_package_workflow() {
        let commands = [
            vec!["donder", "init"],
            vec!["donder", "check"],
            vec!["donder", "add", "library", "alice/library@^1.0"],
            vec!["donder", "remove", "library"],
            vec!["donder", "sync"],
            vec!["donder", "update"],
            vec!["donder", "update", "library"],
            vec!["donder", "tree"],
            vec!["donder", "pack"],
            vec!["donder", "publish"],
            vec!["donder", "publish", "--dry-run"],
            vec!["donder", "login"],
            vec!["donder", "logout"],
            vec!["donder", "whoami"],
            vec!["donder", "fork", "library"],
            vec!["donder", "new", "--from", "alice/template@^1.0"],
        ];
    ...
    fn init_creates_a_manifest_and_deterministic_empty_lock() {
        ...
        fs::write(root.join("project.donder"), "project: {}\n").expect("document");

        init_with_registry(root, "https://registry.donder.dev").expect("init");
        ...
        assert_eq!(lock.registry, "https://registry.donder.dev");
