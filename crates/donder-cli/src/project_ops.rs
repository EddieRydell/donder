...
pub(crate) fn fork(root: &Utf8Path, dependency_alias: &str) -> Result<(), CliError> {
    let forked = donder_package::PackageService::fork_dependency(
        root,
        dependency_alias,
        donder_project_io::validate_registry_package_artifact,
        |candidate| {
            donder_project_io::compile_package_with_cache(
                root,
...
pub(crate) fn new_project(destination: &Utf8Path, package_spec: &str) -> Result<(), CliError> {
    ...
    let resolver_manifest = PackageManifest {
        ...
        exports: BTreeMap::from([(
            "resolver".to_string(),
            ExportGroup {
                documents: vec!["resolver-root.donder".to_string()],
            },
        )]),
...
fn prepare_template_copy(
    ...
    donder_package::copy_package_tree(source, staged)?;
...
fn remove_generated_package_files(root: &Utf8Path) -> Result<(), CliError> {
    ...
    remove_if_file(&root.join(donder_package::LOCK_FILE))
}
...
mod tests {
    use super::*;
    use donder_package::{ProjectManifest, Publication};
    use semver::Version;
    ...
    fn template_copy_is_reidentified_and_clears_publication() {
        ...
        fs::write(source.join("project.donder"), "project: {}\n").expect("project");
        fs::write(source.join(RELEASE_RECEIPT_FILE), "{}").expect("receipt");
        fs::write(source.join(donder_package::LOCK_FILE), "{}").expect("lock");
        ...
        PackageManifest {
            ...
            publication: Some(Publication {
                package: donder_package::PackageId::new("test/template").expect("package"),
        ...
        prepare_template_copy(
            &source,
            &staged,
            &donder_package::PackageId::new("test/template").expect("package"),
        ...
        assert!(!staged.join(donder_package::LOCK_FILE).exists());
    }
}
