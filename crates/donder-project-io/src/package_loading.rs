...
pub struct LoadedPackageProject {
    pub manifest: donder_package::PackageManifest,
    pub lockfile: donder_package::Lockfile,
...
pub struct CompiledSourceGraph {
    ...
    pub project: Option<donder_language::model::DonderProject>,
    pub definitions: donder_language::model::ProjectDefinitionStores,
}
...
pub enum PackageLoadError {
    ...
    Package(donder_package::PackageError),
...
impl From<donder_package::PackageError> for PackageLoadError {
    fn from(error: donder_package::PackageError) -> Self {
        Self::Package(error)
    }
}
...
pub fn load_package(root: &Utf8Path) -> Result<LoadedPackageProject, PackageLoadError> {
    ...
    let project = compiled
        ...
        .ok_or_else(|| LoadProjectError::InvalidEntrypoint {
            path: root.join(donder_package::MANIFEST_FILE),
        })?;
...
pub fn compile_package(root: &Utf8Path) -> Result<CompiledPackage, PackageLoadError> {
    ...
    let lockfile = lockfile.ok_or_else(|| {
        donder_package::PackageError::Invalid("package analysis produced no lockfile".to_string())
    })?;
    Ok(CompiledPackage {
        manifest,
        lockfile,
            ...
            None => {
                push_load_error_diagnostics(
                    ...
                    LoadProjectError::InvalidEntrypoint {
                        path: self.recovery.root.join(donder_package::MANIFEST_FILE),
                    },
                );
                None
            }
    ...
    let result = Loader::new(source_graph).and_then(|mut loader| {
        for (path, text) in overrides {
            ...
            loader.source_overrides.insert(
                donder_language::identity::DocumentId::new(module, path),
                text.clone(),
            );
        }
...
pub fn check_source_graph(source_graph: donder_package::ResolvedSourceGraph) -> ProjectCheckReport {
...
pub fn project_source_texts(root: &Utf8Path) -> io::Result<SourceOverrides> {
    analysis::project_file_inventory(root)
        .into_iter()
        .filter(|path| {
            path.extension() == Some("donder")
                || matches!(
                    path.file_name(),
                    Some(donder_package::MANIFEST_FILE | donder_package::LOCK_FILE)
                )
        })
...
fn analyze_package(root: &Utf8Path) -> (ProjectAnalysis, Option<donder_package::Lockfile>) {
...
fn analyze_package_overrides(
    root: &Utf8Path,
    overrides: &SourceOverrides,
) -> (ProjectAnalysis, Option<donder_package::Lockfile>) {
    let mut diagnostics = Vec::new();
    let manifest = match overrides
        .get(Utf8Path::new(donder_package::MANIFEST_FILE))
        .map_or_else(
            || donder_package::PackageManifest::read_for_analysis(root),
            |text| donder_package::PackageManifest::parse_for_analysis(text.as_bytes()),
        ) {
        Ok(manifest) => {
        ...
        Err(error) => {
            if root.join(donder_package::MANIFEST_FILE).is_file() {
    ...
    let lockfile = match overrides
        ...
        .map_or_else(
            || donder_package::Lockfile::read_for_analysis(root),
            |text| donder_package::Lockfile::parse_for_analysis(text.as_bytes()),
        ) {
        Ok(lockfile) => {
        ...
        match (manifest.as_ref(), lockfile.as_ref()) {
            (Some(manifest), Some(lockfile)) => {
                match package_cache_for_lock(lockfile).and_then(|cache| {
                    donder_package::ResolvedSourceGraph::from_lock(
                        root,
                        manifest.clone(),
                        lockfile,
                        cache.as_ref(),
                    ...
                    Err(error) => {
                        push_diagnostic(
                            &mut diagnostics,
                            IoDiagnostic {
                                path: Utf8PathBuf::from(donder_package::LOCK_FILE),
                                range: None,
                                severity: IoDiagnosticSeverity::Error,
...
pub fn check_package_with_cache(
    ...
    let source_graph = match donder_package::ResolvedSourceGraph::from_lock(
        root,
        manifest,
        &lockfile,
        Some(cache),
...
fn package_cache_for_lock(
    lockfile: &donder_package::Lockfile,
) -> Result<Option<donder_package::CacheStore>, donder_package::PackageError> {
    if lockfile.packages.is_empty() {
        return Ok(None);
    }
    Ok(Some(
        donder_package::DonderDirectories::discover()?.package_cache(),
    ))
}
