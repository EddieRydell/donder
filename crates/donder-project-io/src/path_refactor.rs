...
pub fn plan_path_change(
    ...
    for (module_id, (module_root, _)) in &mutable_modules {
        ...
        if source == relative_root.join(donder_package::MANIFEST_FILE)
            || source == relative_root.join(donder_package::LOCK_FILE)
        {
            return Err(format!(
...
fn prepare_writes(candidate: &ProjectSession) -> Result<BTreeMap<Utf8PathBuf, Vec<u8>>, String> {
    ...
    for module in candidate.source.source_graph.modules().values() {
        ...
        writes.insert(
            module.root.join(donder_package::MANIFEST_FILE),
            donder_package::canonical_json(&module.manifest).map_err(|error| error.to_string())?,
        );
    }
    Ok(writes)
}
...
fn refresh_locks_and_graph(
    ...
    for (module_id, module) in &old_modules {
        ...
        let lock_path = module.root.join(donder_package::LOCK_FILE);
        ...
        write_bytes(
            &lock_path,
            &donder_package::canonical_json(&refreshed).map_err(|error| error.to_string())?,
        )?;
...
fn impacted_manifest_paths(
    ...
    modules
        ...
        .filter_map(|(module_id, (root, manifest))| {
            ...
            (path_changed || owned_path_changed).then(|| {
                relative_root
    ...
    for (path, bytes) in backups.iter().rev() {
        ...
        match bytes {
            Some(bytes) => {
                ...
                donder_package::atomic_write(path, bytes).map_err(io::Error::other)?;
            }
