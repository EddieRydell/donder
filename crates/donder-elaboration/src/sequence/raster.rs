...
//!
//! This module prepares Donder effects and samples their evaluated colors. It does
...
use donder_language::dsl::{BytecodeProgram, DslBindCache, VmWorkspace};
use donder_language::effect::{EffectDefinitionId, EffectInstId};
...
pub struct PreparedEffectRasterRenderer {
    environments: Box<[donder_runtime::bindings::PreparedParameterEnvironment]>,
    frame_rate: u32,
    frame_count: u32,
...
pub struct EffectRasterWorkspace {
    parameters: donder_runtime::bindings::ParameterWorkspace,
    effect_vm: VmWorkspace,
    automation: Vec<Option<donder_runtime::signal::EffectAutomationWorkspace>>,
}
...
impl PreparedEffectRasterRenderer {
    ...
    pub fn start_seconds(&self) -> f32 {
        donder_language::values::sample_time_seconds_f32(self.start_time)
    }

    pub fn duration_seconds(&self) -> f32 {
        donder_language::values::sample_duration_seconds_f32(self.duration)
    }
    ...
    pub fn sampled_raster_column_seconds(
        ...
        Ok(donder_language::values::sample_time_seconds_f32(
            sample_time,
        ))
    }
    ...
    pub fn render_sampled_raster_column_with_workspace(
        ...
        let sample_time = donder_language::values::sample_time_from_seconds_f32(audio_seconds)
...
impl<'a> EffectRasterPrepareBatch<'a> {
    ...
    pub fn prepare_effect(
        ...
        let start_time = donder_language::values::sample_time_from_donder_time(&effect.start)
            .map_err(|_| RenderError::InvalidTiming {
                reason: "effect start exceeds the runtime clock range".to_string(),
            })?;
        let duration =
            donder_language::values::sample_duration_from_donder_duration(&effect.duration)
                .map_err(|_| RenderError::InvalidTiming {
