use donder_language::dsl::{BytecodeProgram, RuntimeError, VmWorkspace};
...
pub(crate) fn render_sampled_effect_target_colors(
    ...
    rendered: &mut [Color],
    sample_time: donder_language::values::SampleTime,
    workspace: &mut VmWorkspace,
    parameters: (
        Option<&mut donder_runtime::signal::EffectAutomationWorkspace>,
        Option<&donder_language::dsl::BoundParams>,
    ),
) -> Result<(), RenderError> {
    effect
        .with_sampler(
