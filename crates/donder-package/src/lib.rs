...
use uuid::Uuid;
use validation::{require_donder_document, valid_alias, validate_relative_path};
pub use validation::{validate_module_relative_donder_path, validate_package_reference_name};
...
pub use config::{DEFAULT_REGISTRY_URL, DonderDirectories, RegistryConfig};
...
pub const MANIFEST_FILE: &str = "donder-package.json";
pub const LOCK_FILE: &str = "donder.lock";
...
impl PackageManifest {
    ...
    fn contract_issues(&self) -> Vec<PackageValidationIssue> {
        ...
        if let Some(project) = &self.project {
            ...
            collect_path_issue(
                ...
                "project.entrypoint",
                &project.entrypoint,
                |value| require_donder_document(value, "project.entrypoint"),
            );
        }
        for (group, export) in &self.exports {
            ...
            for (index, document) in export.documents.iter().enumerate() {
                ...
                collect_path_issue(
                    ...
                    &field_path,
                    document,
                    validate_module_relative_donder_path,
                );
    ...
    pub fn validate_runtime_compatibility(&self) -> Result<(), PackageError> {
        if self.language_version != LANGUAGE_VERSION {
            return Err(PackageError::Invalid(format!(
                "package languageVersion `{}` is incompatible with Donder languageVersion `{LANGUAGE_VERSION}`",
...
impl Lockfile {
    ...
    pub fn validation_issues(&self, manifest: &PackageManifest) -> Vec<PackageValidationIssue> {
        ...
        match manifest_hash(manifest) {
            Ok(expected) if self.manifest_sha256 != expected => {
                push_package_validation_issue(
                    &mut issues,
                    "manifestSha256",
                    "donder.lock does not match donder-package.json",
                );
            }
            Err(error) => {
        ...
        if self.registry.trim().is_empty() {
            push_package_validation_issue(&mut issues, "registry", "donder.lock registry is empty");
        }
    ...
    pub fn validate_local(
        ...
        if current != self.path_dependencies {
            return Err(PackageError::Invalid(
                "donder.lock does not match path dependency content".to_string(),
            ));
        }
        Ok(())
    }
...
fn validate_lock_graph(
    ...
    if visited.len() != graph.len() {
        let unreachable = graph
            ...
            .collect::<Vec<_>>();
        return Err(PackageError::Invalid(format!(
            "donder.lock contains unreachable modules: {}",
            unreachable.join(", ")
        )));
    }
    Ok(())
...
fn visit_lock_node(
    ...
    let dependencies = graph.get(node).ok_or_else(|| {
        PackageError::Invalid(format!("donder.lock points to missing module `{node}`"))
    })?;
...
pub fn pack_directory_with_plan(
    ...
    let publication = manifest.publication.as_ref().ok_or_else(|| {
        PackageError::Invalid("donder pack requires publication metadata".to_string())
    })?;
    if manifest
        .dependencies
        .values()
    ...
    if !plan.files.contains(MANIFEST_FILE) {
        return Err(PackageError::Invalid(
            "release closure must contain donder-package.json".to_string(),
        ));
    }
    if plan.files.contains(LOCK_FILE) || plan.files.contains("donder-release.json") {
        return Err(PackageError::Invalid(
    ...
    if receipt_bytes.len() as u64 > MAX_TEXT_BYTES {
        return Err(PackageError::Invalid(
            "donder-release.json exceeds 1 MB".to_string(),
        ));
    }
    ...
    writer.start_file("donder-release.json", options)?;
...
pub fn inspect_archive(bytes: &[u8]) -> Result<ReleaseReceipt, PackageError> {
    ...
    for index in 0..archive.len() {
        ...
        } else if name == "donder-release.json" {
            receipt = Some(content);
        }
    }
    let manifest_bytes = manifest
        .ok_or_else(|| PackageError::Archive("root donder-package.json is required".to_string()))?;
    ...
    let receipt_bytes = receipt
        .ok_or_else(|| PackageError::Archive("donder-release.json is required".to_string()))?;
...
mod tests {
    ...
    fn manifest(root: &Utf8Path) -> PackageManifest {
        let entrypoint = root.join("main.donder");
        ...
        PackageManifest {
            ...
            project: Some(ProjectManifest {
                entrypoint: "main.donder".to_string(),
            }),
            ...
            exports: BTreeMap::from([(
                String::from("main"),
                ExportGroup {
                    documents: vec!["main.donder".to_string()],
                },
            )]),
            dependencies: BTreeMap::new(),
            assets: BTreeMap::new(),
    ...
    fn lock_detects_manifest_changes() {
        ...
        let value = manifest(root);
        value.write(root).expect("manifest");
        let lock = Lockfile::new(&value, "https://registry.donder.dev").expect("lock");
    ...
    fn resolver_selects_one_release_and_reports_diamonds() {
        ...
        let lock = resolve_registry(root, &value, "https://registry.donder.dev", &available)
            .expect("resolve");
        assert_eq!(
        ...
        assert!(
            resolve_registry(root, &value, "https://registry.donder.dev", &available)
                .expect_err("conflict")
                .to_string()
    ...
    fn shared_protocol_fixtures_match_the_rust_contract() {
        ...
        assert_eq!(
            ...
            "cd2edc3656642995b0fd14e7bebbd10b6c927e31f58519d4f698a2db1fc8388b"
        );
