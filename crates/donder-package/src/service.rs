...
use crate::{
    CacheStore, Dependency, DonderDirectories, LockedDependency, LockedPackage, Lockfile,
...
impl PackageService {
    pub fn prepare(
        ...
        let cache = DonderDirectories::discover()?.package_cache();
        ...
        let (lock, mut deprecated_packages, metadata_loaded) = match &resolution {
            ...
            CandidateResolution::UpdateAlias(alias) => {
                let current = previous_lock.as_ref().ok_or_else(|| {
                    PackageError::Invalid(
                        "a current donder.lock is required for a scoped update; run Sync first"
                            .to_string(),
                    )
                })?;
                if !previous_is_current {
                    return Err(PackageError::Invalid(
                        "donder.lock is stale; run Sync before a scoped update".to_string(),
                    ));
                }
    ...
    pub fn fork_dependency<T>(
        ...
        remove_if_file(&staged.join("donder-release.json"))?;
...
fn rollback_fork<T>(
    ...
    if let Err(error) = original_manifest.write(root) {
        rollback_failures.push(format!("restoring donder-package.json failed: {error}"));
    }
...
mod tests {
    ...
    fn fork_becomes_a_reidentified_path_dependency() {
        ...
        fork.exports = BTreeMap::from([(
            "content".to_string(),
            ExportGroup {
                documents: vec!["main.sequence.donder".to_string()],
            },
        )]);
