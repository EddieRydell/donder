...
use dawn_project_io::{export_editable_project, export_project, load_package};
...
fn imported_show_exports_with_private_effects_nested_imports_and_audio() {
    ...
    let sequence_id = starter
        ...
        .find(|id| {
            ...
            !sequence.effects.is_empty()
                && matches!(
                    sequence.audio,
                    dawn_language::sequence::SequenceAudio::Asset(_)
                )
        })
        .unwrap()
        .clone();
    ...
    let required = starter.project.definitions.effects.definitions[&custom]
        ...
        .map(|param| {
            (
                param.name.clone(),
                dawn_language::effect::EffectParamValue::initial_for_type(&param.ty, color)
                    .unwrap(),
            )
        })
        .collect();
    let effect = &mut starter
        ...
        .unwrap()
        .effects[0];
    effect.definition = dawn_language::effect::EffectRef::Custom(custom.clone());
    ...
    fs::write(
        nested.join("extra.effect.dawn"),
        "effect Extra { color sample() { return hsv(0.0, 1.0, 1.0); } }",
    )
    .unwrap();
    let mut nested_manifest = dawn_package::PackageManifest::read(&rig).unwrap();
    ...
    nested_manifest.exports = BTreeMap::from([(
        ...
        dawn_package::ExportGroup {
            documents: vec!["extra.effect.dawn".into()],
        },
    )]);
    ...
    .unwrap();
    let mut rig_manifest = dawn_package::PackageManifest::read(&rig).unwrap();
    ...
    assert!(
        copied
            ...
            .all(|edge| matches!(
                edge.source(),
                dawn_project_io::ImportSource::LocalDocuments { .. }
            ))
    );
    ...
    let output_before = dawn_elaboration::PreparedSequenceOutput::prepare(
        &original.project,
        &original.project.root.setup,
        &original.project.root.sequences[0],
    )
    .unwrap();
    let output_after = dawn_elaboration::PreparedSequenceOutput::prepare(
        &copied.project,
