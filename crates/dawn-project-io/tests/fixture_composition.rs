...
use dawn_language::layout::{FixtureInstanceId, FixtureTarget, LayoutFixtureKind};
...
fn export_starter(session: &dawn_project_io::ProjectSession, root: &Utf8Path) {
    export_project(session, root).unwrap();
    let source = Utf8PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../examples/starter");
    for name in ["dawn-package.json", "dawn.lock"] {
        std::fs::copy(source.join(name), root.join(name)).unwrap();
    }
    for path in dawn_package::PackageManifest::read(&source)
        .unwrap()
        .assets
        .keys()
    {
...
fn inline_definitions_keep_source_ownership_and_pixel_order() {
    ...
    let root = Utf8Path::from_path(temporary.path()).unwrap();
    export_starter(&session, root);
    let fixture_path = root.join("fixtures/vertical.fixture.dawn");
    ...
    assert_eq!(
        id.0.document(),
        Utf8Path::new("fixtures/vertical.fixture.dawn")
    );
