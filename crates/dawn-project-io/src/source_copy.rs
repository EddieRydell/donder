...
use dawn_language::imports::ImportSource;
...
pub(crate) fn remap_documents(
    ...
    for (old_id, mut document) in std::mem::take(&mut source.documents) {
        ...
        if let SourceDocumentKind::Effect { source: text } = &mut document.kind {
            let imports = dawn_language::dsl::effect_source_imports(text)
...
pub fn export_editable_project(
    ...
    dawn_language::source_remap::remap_document_paths(&mut copied.project, &remaps);
    ...
    manifest.project = Some(dawn_package::ProjectManifest {
    ...
    copied.source.source_graph =
        dawn_package::ResolvedSourceGraph::project(&staged, manifest.clone())
            .map_err(|error| error.to_string())?;
    ...
    let registry = dawn_package::Lockfile::read(session.source.project_root())
        .map_err(|error| error.to_string())?
        .registry;
    dawn_package::Lockfile::new(&manifest, &registry)
        .map_err(|error| error.to_string())?
        .write(&staged)
        .map_err(|error| error.to_string())?;
    ...
    written_files.extend([
        dawn_package::MANIFEST_FILE.into(),
        dawn_package::LOCK_FILE.into(),
    ]);
    Ok(ExportReport {
        written_files,
        copied_assets,
